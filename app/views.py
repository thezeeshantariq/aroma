...
def report_daily(request, day):
    ...
    # Inventory Out
    inventory_out = InventoryOut.objects.filter(created_at__day=day)
...
def report_monthly(request, month):
    # day = int(day)
    context = {}
    context['orders'] = orders = Order.objects.filter(created_at__month=month)
